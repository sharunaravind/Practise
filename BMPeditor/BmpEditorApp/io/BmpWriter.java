...
public class BmpWriter {
    public static void write(String outputFilePath, BmpImage image) throws IOException
    {
        ...
        try (OutputStream os = new BufferedOutputStream(new FileOutputStream(outputFilePath), 65536))
        {
            ...
            byte[] totalPixelArray = new byte[rowSizewithPadding*height];
            int idx;
            for (int y = 0; y < height; y++) {
                int rowStart = y * rowSizewithPadding;
                idx = rowStart;
                for (int x = 0; x < width; x++) {
                    Pixel p = pixelGrid[y][x];
                    totalPixelArray[idx++] = (byte)(p != null ? p.blue : 0);
                    totalPixelArray[idx++] = (byte)(p != null ? p.green : 0);
                    totalPixelArray[idx++] = (byte)(p != null ? p.red : 0);
                }
                for (int p = 0; p < padding; p++) {
                    totalPixelArray[idx++] = 0;
                }
            }
